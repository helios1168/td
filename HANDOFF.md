...
national-only district, and national-only districts must not spread into non-group states.
Grid `grid_20260911_fi21_P5`, 14 of 14 cells, unheld mass 0. The answer: in G1 every group
state gets national-only districts except IL and CO, with TX wholly national-only from k 14; in
G2 IL joins (through IN), CT and from k 14 LA join, and CO, WA, UT, MN get none. The tables,
the reasons and the sweep fix (`8bf4a43`) are in
`battery/results/full_problem/fi21_national_only_summaries/README.md` next to the figures; the
setup and commits in `PLAN.md ## Next step`. Bead `td-9ek.20.8`.
