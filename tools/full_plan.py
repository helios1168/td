...
def _run_passes(problem, passes, args, stage: str, T, *, warm=None,
    ...
    # The stage's first cover pass stopped by the time limit on the empty plan is a time limit
    # with nothing to show, not a plan that covers nothing: fail it as no_incumbent so the grid
    # retries it (on the FI 21 grid, seq_FI went on with cover 0 and the cell came back "ok").
    # Only the first: a later one (cover_merged) at 0 is the normal case once the pure bundles
    # have covered what they can.
    first = next((rec for rec in result["passes"] if rec["name"].startswith("cover")), None)
    if (first is not None and first["status"] == "time_limit"
            and not isinstance(first["value"], dict) and first["value"] <= 1e-9):
        exc = ss.SolveFailure(1, f"{first['name']} stopped at the time limit on an empty plan")
        _write_failure(args.out, stage, exc, time.time() - t0)
        raise exc
    result = dict(result)
