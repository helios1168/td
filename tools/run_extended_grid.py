"""tools/run_extended_grid.py

Solve, assemble, realize, and render the extended grid of candidate scenarios:
- Scenario F (51 Wholesalers): N (12) + WHFI (2) + WH (13) + FI (24)
- Scenario G (52 Wholesalers): N (12) + WHFI (2) + WH (13) + FI (25)
- Scenario H (52 Wholesalers): N (13) + WHFI (2) + WH (13) + FI (24)
- Scenario I (53 Wholesalers): N (13) + WHFI (2) + WH (14) + FI (24)
- Scenario J (53 Wholesalers): N (12) + WHFI (2) + WH (14) + FI (25)
- Scenario K (52 Wholesalers): N (14) + WHFI (1) + WH (13) + FI (24)
- Scenario L (53 Wholesalers): N (14) + WHFI (1) + WH (14) + FI (24)

Appends all runs to scenarios.csv and summary.csv, and renders all summary figures.
"""
...
def assemble_and_realize_scenario(
    ...
    print(f"\n=======================================================")
    print(f"   ASSEMBLING & REALIZING: {scenario_name} (WHFI={k_whfi})")
    print(f"=======================================================")
    ...
    # 4. Dedicated Western Merged Slot(s)
    ...
    if k_whfi == 1:
        ...
        # D1: Plains (ND, SD, NE)
        p_states = ["ND", "SD", "NE"]
        m_p = sum(w_whfi[cells.state_list.index(st)] for st in p_states)
        whfi_slots.append({
            "id": f"P{whfi_base:03d}",
            "bundle": "WHFI",
            "used": True,
            "mass": m_p,
            "contacts": len(p_states),
            "y": {st: 1.0 for st in p_states},
            "L": 100.0, "U": 350.0, "band_hi": 350.0
        })
        # D2: Mountain (CO, ID, MT, WY)
        m_states = ["CO", "ID", "MT", "WY"]
        m_m = sum(w_whfi[cells.state_list.index(st)] for st in m_states)
        whfi_slots.append({
            "id": f"P{whfi_base + 1:03d}",
            "bundle": "WHFI",
            "used": True,
            "mass": m_m,
            "contacts": len(m_states),
            "y": {st: 1.0 for st in m_states},
            "L": 100.0, "U": 350.0, "band_hi": 350.0
        })
...
def main():
    print("=== STARTING EXTENDED MULTI-CHANNEL SCENARIO PIPELINE ===")
    ...
    # Supports for N12 and N14: allow FL to merge with GA/AL
    supports_n_multi = []
    for s in supports:
        names = {state_list[v] for v in s}
        ne_in = names.intersection(NE)
    ...
    # Solve channels in parallel
    print("--- 1. SOLVING CHANNEL MODELS CONCURRENTLY IN PARALLEL ---")
    tau_n14 = tot_n_core / 14
    band_n14 = (tau_n14 * 0.90, tau_n14 * 1.10)
    ...
    channel_tasks = {
        "n14": ("N", w_n, state_list, state_xy, edges, supports_n_multi, 14, band_n14, N_SPLITTABLE, eligible_core, None, 45.0, 0.01),
        "n13": ("N", w_n, state_list, state_xy, edges, supports_n13, 13, band_n13, N_SPLITTABLE, eligible_core, None, 45.0, 0.01),
        "n12": ("N", w_n, state_list, state_xy, edges, supports_n_multi, 12, band_n12, N_SPLITTABLE, eligible_core, None, 45.0, 0.05),
    ...
    print("\nAll underlying channel models solved successfully!\n")

    # Define extended grid scenarios
    scenarios = [
        {
            "id": "scenario_F_51",
            "name": "Scenario F - 51 Wholesalers (N:12, WHFI:2, WH:13, FI:24)",
            "n": n12_res, "wh": wh13_res, "fi": fi24_res, "k_whfi": 2,
            "out_dir": REPO_ROOT / "battery/results/scenario_F_51",
            "fig": REPO_ROOT / "figures/summary_scenario_F.png"
        },
        {
            "id": "scenario_G_52",
            "name": "Scenario G - 52 Wholesalers (N:12, WHFI:2, WH:13, FI:25)",
            "n": n12_res, "wh": wh13_res, "fi": fi25_res, "k_whfi": 2,
            "out_dir": REPO_ROOT / "battery/results/scenario_G_52",
            "fig": REPO_ROOT / "figures/summary_scenario_G.png"
        },
        {
            "id": "scenario_H_52",
            "name": "Scenario H - 52 Wholesalers (N:13, WHFI:2, WH:13, FI:24)",
            "n": n13_res, "wh": wh13_res, "fi": fi24_res, "k_whfi": 2,
            "out_dir": REPO_ROOT / "battery/results/scenario_H_52",
            "fig": REPO_ROOT / "figures/summary_scenario_H.png"
        },
        {
            "id": "scenario_I_53",
            "name": "Scenario I - 53 Wholesalers (N:13, WHFI:2, WH:14, FI:24)",
            "n": n13_res, "wh": wh14_res, "fi": fi24_res, "k_whfi": 2,
            "out_dir": REPO_ROOT / "battery/results/scenario_I_53",
            "fig": REPO_ROOT / "figures/summary_scenario_I.png"
        },
        {
            "id": "scenario_J_53",
            "name": "Scenario J - 53 Wholesalers (N:12, WHFI:2, WH:14, FI:25)",
            "n": n12_res, "wh": wh14_res, "fi": fi25_res, "k_whfi": 2,
            "out_dir": REPO_ROOT / "battery/results/scenario_J_53",
            "fig": REPO_ROOT / "figures/summary_scenario_J.png"
        },
        {
            "id": "scenario_K_52",
            "name": "Scenario K - 52 Wholesalers (N:14, WHFI:1, WH:13, FI:24)",
            "n": n14_res, "wh": wh13_res, "fi": fi24_res, "k_whfi": 1,
            "out_dir": REPO_ROOT / "battery/results/scenario_K_52",
            "fig": REPO_ROOT / "figures/summary_scenario_K.png"
        },
        {
            "id": "scenario_L_53",
            "name": "Scenario L - 53 Wholesalers (N:14, WHFI:1, WH:14, FI:24)",
            "n": n14_res, "wh": wh14_res, "fi": fi24_res, "k_whfi": 1,
            "out_dir": REPO_ROOT / "battery/results/scenario_L_53",
            "fig": REPO_ROOT / "figures/summary_scenario_L.png"
        }
    ]

    print("--- 2. ASSEMBLING & REALIZING EXTENDED SCENARIOS IN PARALLEL ---")
    with ProcessPoolExecutor(max_workers=7) as executor:
        futures = {
            executor.submit(
                assemble_and_realize_scenario,
                sc["id"], sc["name"], sc["n"], sc["wh"], sc["fi"], sc["k_whfi"], sc["out_dir"], sc["fig"]
    ...
    print("\n--- 3. EXPORTING MASTER SCENARIOS CSV FOR ALL 14 SCENARIOS ---")
    # All 14 scenarios: Option 1, Option 2, A, B, C, D, E, F, G, H, I, J, K, L
    all_scenarios = [
