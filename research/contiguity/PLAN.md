...
measurement: the 197-zip pair's optimal log-product moves by std 8.4e-3 nats (24/197 zips flip)
under the contestability noise model with θ/λ fixed, so 5e-3 is below the data-noise floor while
the worst current residual (3.9e-3) passes with headroom. Harness: `base.EPS_CERT`,
`eps_certified` row flag, `eps_certified_frac` summary column. S3's T3 pass criterion is tier 2;
dual-side work (deeper separation, E1) is an improvement track, not a blocker. Re-measure the
floor on the twin when it lands. (ii) Adjacency weakening (Queen/island bridges) assessed:
a data-prep question for the real graph and a legitimate relaxation bound, not a solver lever —
optional small experiment queued behind the twin. (iii) **Math note merged** (`e87867e`):
`research/contiguity/math_note/` — standalone LaTeX companion (model, gain-space geometry, OA,
root-free cuts, `scip_tree`, two enumerable toys with generated-macro numbers). Earlier —
**State on 2026-08-30 (19:00):** primal/dual diagnostic done (`RESULTS.md` §Primal/dual): ILS beats scip_tree's large-pair incumbents in ≤ 1.4 s (+1e-3 on 197/464) then plateaus at what two independent starts agree is the optimum; scip_tree warm-started *at that optimum* still cannot close the bound in 1200 s (169/197 stay at 3–4e-3; 205/320 nearly close at 1.35e-4/2.4e-5). **The 160+ residual is dual, not primal** — W6d shrinks to wiring (`_local_search` to convergence on incumbents); the real levers are E1 reductions, deeper fractional separation, or accepting ≤ 0.4 % certified gaps. Earlier — **State on 2026-08-30 (17:00):** **W6b merged** (`930ae55`; `RESULTS.md` §W6b): `scip_tree`'s S2 scale wall was an unused-budget bug — an LP abort was reported as `time_limit`, so the retry ladder never ran. With the ladder running (plus `warm_f1` MIP start, root fractional separation, `_short_stop` for SCIP's clock under load), all seven 124–464-zip pairs improve at 1200 s: 124/135 zips reach 1.1e-7 / 3.0e-8 (the loosened rung's tolerance floor, stopping early), 205/320 reach 3e-4 / 1e-4, 464 reaches 2e-3; 169/197 stay at 4e-3 (primal-side). No new certificates at `CERT_TOL`. **Next:** S3 when the twin lands (`scip_tree` + `scip_tree_psimplex` + `flow_pwl`, 3600 s); candidate units W6c (exact post-hoc certificate for the 1e-7 residual) and an in-tree primal for 160+ zips — user's call at ★. U12 merged (`e0a333d`: instance cards share one tessellation; S2 instance JSONs lack `context`, re-emit via `--phase 1` before rendering S2 cards). Earlier — **State on 2026-08-30 (12:00):** **S2 done** (`RESULTS.md` §S2): at 1200 s every exact finalist certifies all pairs ≤ 45 zips, 2/3 at 83–125, **0/6 at 126–464** — identical to the 60 s picture; `scip_tree` stops in seconds on large pairs (retry ladder exhausts on LP aborts, OA rung not given the remaining time), `flow_pwl` uses the budget and reaches 1e-4–2e-4. **Next: W6b — scip_tree primal/numerics follow-up** (spend remaining time on the OA rung after a native abort; `warm_f1` via `addSol`; fractional separation at the root; re-measure on the six 126–464 pairs), then S3 once the twin (T3) lands. Graphics: U10/U11 merged (context panel, log-ratio map). Earlier — **State on 2026-08-30 (04:00):** **S1 screening done** (`RESULTS.md` §S1; page https://claude.ai/code/artifact/a4ab645f-abae-4418-9817-9231aa33f1cf): `scip_tree` 92 % of T1 certified (median 0.08 s), `flow_pwl`/`loop_v2` 78 %, `flow` 73 %; all four agree where they certify; C7 A3/B3 (205) and the 320/464 pairs are the open scale cases. `cbc_tree` demoted to heuristic (false certificates + hung jobs). U10 merged (gfx↔harness reconciliation). **Next ★: user picks S2/S3 finalists** (recommended: `scip_tree`+`warm_f1` start, `flow_pwl`, `loop_v2`; `current_tu` ρ=2e-3 continuity column), then S2 overnight (T1+T2+T3 at 1200 s) once the twin lands (T3) — or T1+T2 only now. Earlier — **State on 2026-08-30 (00:xx):** everything in the unit wave and the method wave is **merged** on `contiguity-harness` (U1a–U9, W4–W9a; 203 fast tests green). Full S0 in `RESULTS.md` §"S0 full": `scip_tree`, `loop_v2`, `flow`, `flow_pwl` each certify 13/13 T0 and 5/6 named failures; only C7 A3/B3 (205) is open. **Next: ★3 with the user for S1** (proposed methods: brute, current_tu, scip_tree, loop_v2, flow, flow_pwl, cbc_tree, warm_f1, prep_e1_flow on T0+T1+T2 at 60 s). The twin-export tool is ready for the work machine (`tools/twin_export/README.md`). Earlier — **State on 2026-08-29 (late):** on `contiguity-harness` (`566e40d`+): U0a–U1a, U1b, U2, U4, U5, U7, **W5 (warm), W8 (prep) merged**; W6 build (SCIP, plan certified 5/6 named failures in ≤ 2 s), W4 build (flow/flow_pwl), W7 (CBC), W9a (loop_v2 + lexi), U3 (twin export) still on `wt/*` — check `git worktree list`; merge each after `run_all.py` is green in its worktree, then S0 rerun with all methods (`contiguity_bench.py --stage S0`), then ★3 with the user before S1. Trap 14 (HiGHS 1e-9 tolerance solve error) fixed in `territory.py`; zip50 anchor deliberately refreshed. Earlier: U0a–U1a, **U1b, U2, U4, U7 merged**;
...
| **Acceptance (decided 2026-08-30)** | **Two-tier:** tier 1 `CERT_TOL=1e-8` on T0/T1 + named failures (W6c to make the 124–135 rows rigorous); tier 2 `EPS_CERT=5e-3` nats for T2+, ε-certified, read with cross-method primal agreement. Overrides `TEST_PLAN.md` §3/§7 "all pairs ≤ 1e-8" at production sizes. Evidence and ε derivation: `RESULTS.md` §Two-tier (noise-floor bootstrap). |
