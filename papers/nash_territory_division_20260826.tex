...
The Nash bargaining solution maximises the product of gains,
$\max_S\ g_a(S)\,g_b(S)$ subject to $g_a,g_b>0$. Define the utility ratio
\begin{equation}
...
\subsection{The exact method}\label{sec:exact}
...
exact method by default}; the prefix table is a fast preview and a sanity check.
\end{resultbox}
...
\paragraph{Exactly solvable.} Proposition~\ref{prop:prefix} makes the optimum reachable
in one pass. The equalisation criteria require search for their exact optima.
...
Nash, prefix heuristic & $25$ & $4.9360$ & $4.8807$ & $24.09096$\\ \bottomrule
...
\section{Where the prefix property fails}\label{app:prefix}
