...
\paragraph{No feasibility threshold.} Nash requires $g_a,g_b>0$. At $d=(0,0)$ this holds
automatically for any non-trivial split (Section~\ref{sec:statement}), since gains are
bare bundle utilities and $\theta,\lambda\in[0,1]$ keep them non-negative pointwise. This
is not true of the discarded pre-merger baseline, where headroom below a threshold
$\lambda^\ast$ --- $0.069$ on the instances tested, under the net convention --- left the
bargaining set empty and forced a fallback to the egalitarian criterion. That threshold
was an artefact of measuring gains against pre-merger production; it does not arise here.
