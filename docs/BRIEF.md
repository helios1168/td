...
|---|---|---|---|
| **★6** | **Lifted in full 2026-09-03** (user; A1 track, `wt/A1`) — units may run code against the instance for any purpose; U7-meas launched. *Original:* **May a unit run code against the instance?** This plan's own constraint says no; both domains' plans open with measurement, the instance is not in this worktree, and every number in `DOMAIN_optimization.md` §5 and `DOMAIN_economic-theory.md` §5 is blocked until it is lifted. | **U7-meas entirely**, plus the conditional clauses in all three wave-1 units | user (programme) |
