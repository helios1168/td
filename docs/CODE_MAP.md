...
| `tools/instance_conus.py` | a raw export -> its CONUS-only cut: keeps the 49 CONUS+DC state codes, drops the rest, induces the edges, records `meta["conus_filter"]`, then reports the per-channel mass, `fine_split` fallback count, tau(k=18) and per-bundle slot counts. Column-generic, so it needs no change between a `national` file and a sub-channel one. `instance_conus.py SRC DST`, defaults to the v4 export and its CONUS cut at the hub root |
| `tools/instance_split_national.py` | national -> its three sub-channels at fixed shares (default chase 0.5 / wells_wh 0.3 / wells_fi 0.2), applied to `M_c`, every rep's `S_c` and `S_free_c` alike; a stand-in for the real v4 export, used to build `instance_descaled_v4synth_conus.json.gz` from the v3 CONUS file and test the pipeline before v4 lands |
| `tools/full_problem_runs/` | the full-problem track's run scripts (2026-09-11): `cells.py` writes `tools/full_grid.py` cell files with the forced-national rules, `run_grid.sh` runs a grid through re-realise and `force_check.py`, plus `rerealise.sh`, `probe_force.py`, `compare.py`, `unrepaired.py`; paths from `TD_ROOT`. The track's `HANDOFF.md` says how they are used |
