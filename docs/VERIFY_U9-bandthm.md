...
`docs/foundations/FRAME.md`, `docs/foundations/BRIEF.md`, `docs/foundations/APPROACHES.md`, `docs/foundations/LENS_*`, `docs/foundations/DOMAIN_*`, `docs/foundations/LIT_*`.
...
4. **A citation slip in P1b-band.** A1's `docs/foundations/LENS_GROMOV.md` M8 (line 58) **already**
...
`docs/foundations/DOMAIN_optimization.md:420` states `supp(X) ⊆ argmax_i u_i(z)/(p_z + ν_i M_z)`.
...
1. **§2 P1b-band / §3.** "`LENS_GROMOV` M8's `V ≤ EG^bal_{S₁₃}(0.0078)`" is out of date. A1's
   `docs/foundations/LENS_GROMOV.md:58` already writes the sandwich with `δ₀`, and lines 62–64 already
   carry the max-deviation-vs-spread distinction. The live `0.0078` sites are M8's U13 grid
   (lines 118, 121, 155, 271) and `docs/foundations/DOMAIN_economic-theory.md` lines 440–441, 520, 739.
...
### To `docs/foundations/DOMAIN_optimization.md` (for whoever owns it)
